/*
 ...
 The macro DIM corresponds to the number of dimensions in a space.
 ...
 calculations - it has DIM elements. In each element, only one bit
 is zero valued - the top bit in element no. 0 and the bottom bit
 in element no. (DIM - 1). eg.
 if DIM == 5 const U_int g_mask[]
 if DIM == 6 const U_int g_mask[]
 etc...
 */
...
U_int calc_P (int i, Hcode H)
{
    ...
    if (i % ORDER > ORDER - DIM) {
        ...
        /* the & masks out spurious high bit values */
        #if DIM < ORDER
            P &= (1 << DIM) -1;
        #endif
    return P;
